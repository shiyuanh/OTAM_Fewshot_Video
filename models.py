...
TSN Configurations:
    ...
    def forward(self, input, feat_only=True):
        sample_len = (3 if self.modality == "RGB" else 2) * self.new_length
        #base_out = self.base_model(input.view((-1, sample_len) + input.size()[-2:]))
        base_out = self.base_model(input)

        if self.dropout > 0 and (not feat_only):
...
class Fusion(nn.Module):
    def __init__(self, in_dim = 512, mode='cat'):
        super(Fusion, self).__init__()
        self.mode = mode
        if self.mode == 'mlp':
            self.mlp = nn.Sequential(nn.Linear(in_dim*2, in_dim), nn.ReLU())

    def forward(self, feat_rgb, feat_flow):
        if self.mode == 'cat':
            return torch.cat([feat_rgb, feat_flow], dim=-1)
        elif self.mode == 'avg':
            return (feat_rgb + feat_flow) / 2
        elif self.mode == 'mlp':
            feat_in = torch.cat([feat_rgb, feat_flow], dim=-1)
